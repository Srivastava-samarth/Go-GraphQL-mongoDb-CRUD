...
func (db *DB) GetJob(id string) *model.JobListing{
	...
	var jobLisiting model.JobListing
	err := jobCollection.FindOne(ctx,filter).Decode(&jobLisiting)
	if err != nil{
		log.Fatal(err)
	}
	return &jobLisiting
...
func (db *DB) GetJobs() []*model.JobListing{
	...
	ctx,cancel := context.WithTimeout(context.Background(),30*time.Second)
	defer cancel() 
	var jobLisitings []*model.JobListing
	cursor,err:=jobCollection.Find(ctx,bson.D{})
	if err!=nil{
		log.Fatal(err)
	}
	if err = cursor.All(context.TODO(),&jobLisitings); err!=nil{
		panic(err)
	}
	return jobLisitings
}
...
func (db *DB) CreateJobListing(jobInfo model.CreateJobListingInput) *model.JobListing{
	...
	ctx,cancel := context.WithTimeout(context.Background(),30*time.Second)
	defer cancel()
	inserg,err := jobCollection.InsertOne(ctx,bson.M{"title":jobInfo.Title, "description":jobInfo.Description, "url":jobInfo.URL, "company":jobInfo.Company})
	if err!=nil{
		log.Fatal(err)
	}
...
func (db *DB) UpdateJobListing(jobId string,jobInfo model.UpdateJobListingInput) *model.JobListing{
	...
	var jobLisiting model.JobListing
	if err := results.Decode(&jobLisiting); err!=nil{
		log.Fatal(err)
	}
	return &jobLisiting
}

func (db *DB) DeleteJobLisitng(jobId string) *model.DeleteJobResponse{
	jobCollection := db.client.Database("graphql-job-board").Collection("jobs")
	ctx,cancel := context.WithTimeout(context.Background(),30*time.Second)
	defer cancel()
